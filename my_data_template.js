...
(function() {
	...
	var RandomData = function() {
		...
		var generateDetermWorkItems = function () {
			var data = []
			var scale_const = 100


			for (var i = 0; i < my_data.length - 1; i++) {
				for (var j = 0; j < my_data[i].length - 1; j++) {
					var workItem = {
						id: my_data[i][j].id,
						name: 'work item ' + my_data[i][j].name,
						lane: 'lane ' + i,
						start: new Date(2015, 1, my_data[i][j].begin / scale_const, 16, 0, 0),
						end: new Date(2015, 1, my_data[i][j].end / scale_const, 16, 0, 0),
						desc: 'This is a description.'
					};
					data.push(workItem);
				}
		...
		return parseData(generateDetermWorkItems());
	};
