...
######## MAIN ############
def do(reference, filenames, aligned_lengths_lists, nucmer_dir, output_dir, all_pdf, draw_plots, results_dir):
    ...
    import N50
    for id, (filename, lens, assembly_len) in enumerate(itertools.izip(
            filenames, aligned_lengths_lists, assembly_lengths)):
        ...
        report.add_field(reporting.Fields.LARGALIGN, max(lens), reference)
        report.add_field(reporting.Fields.NA50, na50, reference)
        report.add_field(reporting.Fields.NGA50, nga50, reference)
        report.add_field(reporting.Fields.NA75, na75, reference)
        report.add_field(reporting.Fields.NGA75, nga75, reference)
        report.add_field(reporting.Fields.LA50, la50, reference)
        report.add_field(reporting.Fields.LGA50, lga50, reference)
        report.add_field(reporting.Fields.LA75, la75, reference)
        report.add_field(reporting.Fields.LGA75, lga75, reference)

    ########################################################################

    json_saver.save_aligned_contigs_lengths(filenames, aligned_lengths_lists)
    json_saver.save_assembly_lengths(filenames, assembly_lengths)
    html_saver.save_aligned_contigs_lengths(filenames, aligned_lengths_lists)
    html_saver.save_assembly_lengths(filenames, assembly_lengths)
