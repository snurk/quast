...
def unscaled_upper_mad(xs):
    ...
    """
    med = median(xs)
    above_med = [(x-med) for x in xs if x > med]
    return med, median(above_med)
...
L = [x for x in L if x < upper_cutoff]
new_len = len(L)
removed = c - new_len
...
mean = sum(L)/len(L)
L2 = [x**2 for x in L]
stdev = math.sqrt((sum(L2) / len(L)) - (mean * mean))
...
print 'mean %s stdev %.2f' % (mean, stdev)
